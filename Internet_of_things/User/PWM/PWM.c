...
void TIM1_PWM_Init(u16 arr,u16 psc) //初始化pwm输出引脚  定时器1
{	
  ...
  //设置该引脚为复用输出功能,输出PUL的PWM脉冲波形
	...
	TIM_OCInitStructure.TIM_Pulse = arr>>1;//50% //比较tim_ccr的值，输出脉冲发生跳变
  ...
  TIM_CtrlPWMOutputs(TIM1,ENABLE);	//MOE 主输出使能	
	...
	TIM_Cmd(TIM1, ENABLE);  //使能PUL_TIM              
}

void TIM8_PWM_Init(u16 arr,u16 psc) 	//初始化pwm输出引脚  定时器8
{	
	GPIO_InitTypeDef GPIO_InitStructure;
	TIM_TimeBaseInitTypeDef  TIM_TimeBaseStructure;
	TIM_OCInitTypeDef  TIM_OCInitStructure;
